...
covarM = (trueMean' * trueMean)/(numTrainingFiles);

[eig_vect,eig_value] = eig(covarM);
...
sorted_eigenfaces = trueMean * eig_vecs_sorted; 


%Normalize eigenfaces
normalizedPC = zeros(dimension , numTrainingFiles);

for i = 1:numTrainingFiles
    normalizedPC(: , i) = sorted_eigenfaces(: , i) / norm(sorted_eigenfaces(: , i));
end
...
for i = 1:num_test_images_to_display
    ...
    weights = normalizedPC' * centered_test_col;

    reconstructed_col = meanFace1 + normalizedPC(:, 1:k) * weights(1:k);
...
test_image_index = 166;  % <<== CHANGE THIS TO THE TEST IMAGE YOU WANT
...
for k = 1:numTrainingFiles
    weights_k = normalizedPC(:, 1:k)' * centered_test_col;
    reconstruction_k = meanFace1 + normalizedPC(:, 1:k) * weights_k;
